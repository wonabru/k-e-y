...
func ShowDexPage() *widgets.QTabWidget {
	...
	addPoolButton.ConnectClicked(func(bool) {
		...
		if qad != "" {
			...
			if amount != "" {
				...
				priceToken.SetText(fmt.Sprintf("Pool price %s/QWIDT = %f", symbol, price))
				if poolCoin > 0 {
					priceBid = common.CalcNewDEXPrice(0, poolToken, poolCoin)
					poolPriceToken.SetText(fmt.Sprintf("My Price %s/QWIDT = %f", symbol, priceBid))
				}
	...
	removePoolButton.ConnectClicked(func(bool) {
		...
		if amount != "" {
			...
			if poolCoin > 0 {
				...
				poolPriceToken.SetText(fmt.Sprintf("My Price %s/QWIDT = %f/%f", symbol, priceAsk, priceBid))
			}
			priceToken.SetText(fmt.Sprintf("Pool price %s/QWIDT = %f", symbol, priceAsk))

		}
	})

	removePoolButtonQAD = widgets.NewQRadioButton2("Withdraw QWIDT", nil)
	widget.Layout().AddWidget(removePoolButtonQAD)
	removePoolButtonQAD.ConnectClicked(func(bool) {
		poolTokensButton.SetText("Withdraw QWIDT from Pool")
	...
	amountTokens.ConnectTextChanged(func(amount string) {
		if amount != "" {
			QWIDT := amountQAD.Text()
			if QWIDT != "" {
				g, _ := strconv.ParseFloat(QWIDT, 64)
				t, _ := strconv.ParseFloat(amount, 64)
	...
	amountQAD.SetPlaceholderText("Amount of QWIDT")
	widget.Layout().AddWidget(amountQAD)
	amountQAD.ConnectTextChanged(func(QWIDT string) {
		if QWIDT != "" {
			amount := amountTokens.Text()
			if amount != "" {
				g, _ := strconv.ParseFloat(QWIDT, 64)
	...
	priceToken.SetPlaceholderText("Price of token in QWIDT")
	priceToken.SetEnabled(false)
	widget.Layout().AddWidget(priceToken)

	poolPriceToken = widgets.NewQLineEdit(nil)
	poolPriceToken.SetPlaceholderText("Price of token you get in QWIDT")
...
func GetAllPoolsInfo() string {
	...
	for addr, info := range ti {
		...
		txt += fmt.Sprintln("Users provided liquidity into pool: ", coinPoolFloat, " QWIDT")
		if tokenPoolFloat > 0 {
			price = common.RoundCoin(coinPoolFloat / tokenPoolFloat)
		}
		txt += fmt.Sprintf("Pool price %s/QWIDT = %f", symb, price)
...
func GetAllTokensAccountInfo(a common.Address, symbolAddr common.Address) string {
	...
	for addr, info := range ti {
		...
		if bal, ok := accDex.Balances[myacc.Address]; ok {
			...
			txt += fmt.Sprintln(addr, " = ", humanReadableQAD, " QWIDT")
		}
	}
...
func MakeTransaction(sender, coinAddr common.Address, primary bool) {
	...
	if ok {
		...
		QWIDT, err := strconv.ParseFloat(amountQAD.Text(), 64)
		if err != nil {
			v = fmt.Sprint(err)
			info = &v
			return
		}
		QADam := int64(QWIDT * math.Pow10(int(common.Decimals)))
		operation := 2
		...
		//	v = fmt.Sprint("Not enough QWIDT balance at account")
		//	info = &v
		//	return
		//}
...
//	chart.SetTitle(fmt.Sprintf("Prices of QWIDT/%s from DEX", symbol))
